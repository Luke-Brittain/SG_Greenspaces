...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if gdf is not None:
        ...
        # Build DATA lookup from live dataframe (always reflects current CSV)
        import json as _json
        _map_lookup = {}
        for _, _r in df.iterrows():
            _map_lookup[str(_r["PLN_AREA_N"])] = {
                "name":   str(_r["name"]),
                "region": str(_r["region"]),
                "pop":    int(_r["pop2020_total"]) if pd.notna(_r.get("pop2020_total")) and int(_r.get("pop2020_total", 0) or 0) > 0 else None,
                "gr":     round(float(_r["pct_green_res"]), 1) if pd.notna(_r.get("pct_green_res")) else 0,
                "pk":     round(float(_r["pct_parkland"]),  1) if pd.notna(_r.get("pct_parkland"))  else 0,
                "ur":     round(float(_r["pct_urban"]),     1) if pd.notna(_r.get("pct_urban"))     else 0,
                "wa":     round(float(_r["pct_water"]),     1) if pd.notna(_r.get("pct_water"))     else 0,
                "gub":    round(float(_r["gub"]), 3) if pd.notna(_r.get("gub")) else None,
                "lgs":    round(float(_r["lgs"]), 1) if pd.notna(_r.get("lgs")) else None,
            }
        _map_data_js = _json.dumps(_map_lookup, separators=(",", ":"))
